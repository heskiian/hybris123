...
/**
 * Resolves SOLR localized name property for the product given a {@link PriceRowModel}.
 *
 * @since 2105
 */
public class TmaPoNameResolver extends TmaAbstractLocalizedResolver
{
	private static final Logger LOG = Logger.getLogger(TmaPoNameResolver.class);
	private TmaSpoSource tmaSpoSource;

	public TmaPoNameResolver(final TmaSpoSource tmaSpoSource, final CommonI18NService commonI18NService)
	{
	...
	{
		final String name =
				getTmaSpoSource().getProduct((PriceRowModel) item).getName(getCommonI18NService().getLocaleForLanguage(language));
		if (!StringUtils.isBlank(name))
		{
			try
			{
				document.addField(indexedProperty, name, language.getIsocode());
			}
