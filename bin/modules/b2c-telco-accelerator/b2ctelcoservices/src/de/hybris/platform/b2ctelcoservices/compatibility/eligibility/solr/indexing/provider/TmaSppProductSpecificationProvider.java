...
/**
 * Value provider that handles indexing of productSpecification.Id property from data model to Solr.
 *
 * @since 1903
 */
public class TmaSppProductSpecificationProvider extends TmaAbstractSppProvider
{
	/**
	 * Creates and returns a list of fieldValues that contains the productSpecification property of the model.
	 * @param indexedProperty The indexed property
	 * @param model The object whose value is indexed
	 * @return A list of fieldValues which contain the model's productSpecification value.
	 */
	@Override
	protected Collection<FieldValue> getValues(IndexedProperty indexedProperty, Object model)
	{
		...
		if (model instanceof TmaSimpleProductOfferingModel)
		{
			...
			{
				productSpecification = productOffering.getProductSpecification().getId();
			}
		}
		addFieldValues(fieldValues, indexedProperty, null, productSpecification);
		return fieldValues;
	}
}
