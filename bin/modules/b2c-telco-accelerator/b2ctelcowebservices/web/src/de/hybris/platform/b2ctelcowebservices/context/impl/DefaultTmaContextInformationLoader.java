 ...
 */
package de.hybris.platform.b2ctelcowebservices.context.impl;

import de.hybris.platform.b2ctelcowebservices.constants.B2ctelcowebservicesConstants;
import de.hybris.platform.b2ctelcowebservices.context.TmaContextInformationLoader;
 ...
 * @since 1810
 */
public class DefaultTmaContextInformationLoader implements TmaContextInformationLoader
{
	private static final String[] urlSplitters =
	{ "/v1/", "/v2/" };

	private static final Logger LOG = Logger.getLogger(DefaultTmaContextInformationLoader.class);
	...
	{
		final String languageString = request.getParameter(B2ctelcowebservicesConstants.HTTP_REQUEST_PARAM_LANGUAGE);
		...
		if (!StringUtils.isBlank(languageString))
		{
			...
			{
				throw new TmaUnsupportedLanguageException("Language  " + YSanitizer.sanitize(languageString) + " is not supported",
						e);
			}
		}
		...
		if (currentBaseStore != null)
		{
			...
			{
				throw new TmaUnsupportedLanguageException("Current base store supports no languages!");
			}

			if (!storeLanguages.contains(languageToSet))
			{
				throw new TmaUnsupportedLanguageException(languageToSet);
			}
		}
	...
	{
		final String currencyString = request.getParameter(B2ctelcowebservicesConstants.HTTP_REQUEST_PARAM_CURRENCY);
		...
		if (currentBaseStore != null)
		{
			...
			{
				throw new TmaUnsupportedCurrencyException("Current base store supports no currencies!");
			}
	 ...
	 */
	protected void recalculateCart(final String currencyString) throws TmaRecalculationException
	{
		if (getCartService().hasSessionCart())
		{
			...
			if (cart != null)
			{
				...
				{
					throw new TmaRecalculationException(e, YSanitizer.sanitize(currencyString));
				}
			}
		}
	}
	 ...
	 */
	@Override
	public BaseSiteModel initializeSiteFromRequest(final HttpServletRequest request) throws TmaInvalidResourceException
	{
		...
		{
			throw new TmaInvalidResourceException(YSanitizer.sanitize(baseSiteUid));
		}
	...
	protected String[] getSpecialUrlCharacters()
	{
		final String configurationString = getConfigurationService().getConfiguration().getString(
				B2ctelcowebservicesConstants.URL_SPECIAL_CHARACTERS_PROPERTY,
				B2ctelcowebservicesConstants.DEFAULT_URL_SPECIAL_CHARACTERS);
