...
class DefaultOutboundSyncJobRegisterUnitTest extends Specification {
    ...
    def 'retrieves all running jobs'() {
        ...
        def job1 = jobRegister.getNewJob outboundSyncJob(PK.fromLong(1))
        and: 'another job is running'
        def job2 = jobRegister.getNewJob outboundSyncJob(PK.fromLong(2))
    ...
    def 'retrieved running jobs are immutable'() {
        given: 'a job is running'
        jobRegister.getNewJob outboundSyncJob(PK.fromLong(1))

        when: 'the running jobs collection is attempted to be modifed'
    ...
    @Test
    @Unroll
    def "the job is running after #method was called"() {
        given: 'the job is requested'
        jobRegister."$method" outboundSyncJob(EXISTING_JOB_PK)

        expect:
        jobRegister.isRunning(EXISTING_JOB_PK)

        where:
        method << ['getNewJob', 'getJob']
    }
