...
/**
 * Implementation of {@link TmaValidationMessagesStrategy} to set the list of validation objects
 */
public class DefaultTmaCompatibilityValidationMessagesStrategy implements TmaValidationMessagesStrategy
{
	...
	public DefaultTmaCompatibilityValidationMessagesStrategy(final TmaCartValidationBuilder cartValidationBuilder,
	...
	public void setValidationMessagesOn(final CartModel cartModel, final List<String> messages)
	{
		...
		if (cartModel.getCartValidations() == null)
		{
			cartModel.setCartValidations(new HashSet<>());
		}

		final Set<TmaCartValidationModel> cartValidations = new HashSet<>(cartModel.getCartValidations());
		cartValidations.addAll(getCartValidationBuilder().createCartValidations(COMPATIBILITY, messages));
		cartModel.setCartValidations(cartValidations);
	...
	{
		return CollectionUtils.isNotEmpty(entryGroup.getValidationMessages()) && !entryGroup.getValidationMessages().stream()
	...
	{
		return cartEntry != null && cartEntry.getCartEntryValidations() != null && !cartEntry.getCartEntryValidations().stream()
				.map(TmaCartValidationModel::getMessage).collect(Collectors.toSet()).containsAll(errorMessages);
	}
	...
	public void setValidationMessagesOn(final EntryGroup entryGroup, final List<String> messages)
	{
		...
		entryGroup.getValidationMessages().addAll(getCartValidationBuilder().createCartValidations(COMPATIBILITY, messages));
	}
	...
	public void setValidationMessagesOn(final CartEntryModel cartEntry, final List<String> messages)
	{
		...
		validationMessages.addAll(getCartValidationBuilder().createCartValidations(COMPATIBILITY, messages));
	...
	@Override
	public void cleanupValidationMessagesOn(CartModel cartModel)
	{
		if (CollectionUtils.isEmpty(cartModel.getCartValidations()))
		{
			return;
		}

		final List<TmaCartValidationModel> compatibilityValidations = cartModel.getCartValidations().stream()
				.filter((TmaCartValidationModel validation) -> COMPATIBILITY.equals(validation.getCode()))
				.collect(Collectors.toList());

		cartModel.setCartValidations(cartModel.getCartValidations().stream()
				.filter((TmaCartValidationModel validation) -> !COMPATIBILITY.equals(validation.getCode()))
				.collect(Collectors.toSet()));

		getModelService().save(cartModel);
		getModelService().removeAll(compatibilityValidations);
	}
	...
	public void cleanupValidationMessagesOn(final AbstractOrderModel abstractOrder, final EntryGroup entryGroup)
	{
		...
		final List<TmaCartValidationModel> compatibilityValidations = entryGroup.getValidationMessages().stream()
				.filter((TmaCartValidationModel validation) -> COMPATIBILITY.equals(validation.getCode()))
				.collect(Collectors.toList());

		entryGroup.setErroneous(entryGroup.getValidationMessages().stream()
				.anyMatch((TmaCartValidationModel validation) -> validation != null && !COMPATIBILITY.equals(validation.getCode())));
	...
	public void cleanupValidationMessagesOn(final CartEntryModel cartEntry)
	{
		...
		final List<TmaCartValidationModel> compatibilityValidations = cartEntry.getCartEntryValidations().stream()
				.filter((TmaCartValidationModel validation) -> COMPATIBILITY.equals(validation.getCode()))
				.collect(Collectors.toList());
