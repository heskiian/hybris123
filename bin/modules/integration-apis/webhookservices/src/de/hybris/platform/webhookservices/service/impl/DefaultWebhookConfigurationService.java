...
public class DefaultWebhookConfigurationService implements WebhookConfigurationService
{
	...
	private Collection<WebhookConfigurationModel> findByEventAndItemModel(final AbstractEvent event, final ItemModel item)
	{
		...
		loadLazyLoadedModels(webhookConfigurationModels);
		return webhookConfigurationModels;
	}

	private void loadLazyLoadedModels(final Collection<WebhookConfigurationModel> webhookConfigurationModels)
	{
		webhookConfigurationModels.forEach(model -> model.getDestination().getDestinationTarget().getId());
	}
