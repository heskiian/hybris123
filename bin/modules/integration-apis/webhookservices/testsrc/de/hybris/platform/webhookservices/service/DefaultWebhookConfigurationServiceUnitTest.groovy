...
class DefaultWebhookConfigurationServiceUnitTest extends Specification {
    ...
    @Test
    @Unroll
    def "getWebhookConfigurationsByEventAndItemModel loaded #numberOfModels models before returning"() {
        given: "Found #numberOfModels configurations with matching event and root item"
        def config = webhookConfigWithIntObjRootItem(rootItemOfType('Order'))
        flexibleSearch.search(_ as FlexibleSearchQuery) >> Stub(SearchResult) {
            getResult() >> [config] * numberOfModels
        }
        descriptorFactory.createItemTypeDescriptor({ it.type.code == 'Order' }) >> Stub(TypeDescriptor) {
            isInstance(_ as OrderModel) >> true
        }
        ...
        when: 'Searching for an event with an Integration Object root item of Order type'
        service.getWebhookConfigurationsByEventAndItemModel Stub(AbstractEvent), new OrderModel()

        then: 'All models are eagerly loaded'
        numberOfModels * destinationTarget.getId()

        where:
        numberOfModels << [0, 1, 2]
    }
