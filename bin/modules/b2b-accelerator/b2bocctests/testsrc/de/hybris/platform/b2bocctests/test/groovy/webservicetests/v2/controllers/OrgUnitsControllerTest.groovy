...
class OrgUnitsControllerTest extends AbstractUserTest {
	...
	def "B2B Admin gets the root organizational unit node"() {
		...
		def response = restClient.get(
				path: getBasePathWithSite() + '/users/' + B2B_ADMIN.id + '/orgUnitsRootNodeTree',
				contentType: JSON,
	...
	def "B2B Customer tries to get the root organizational unit node"() {
		...
		def response = restClient.get(
				path: getBasePathWithSite() + '/users/' + B2B_CUSTOMER.id + '/orgUnitsRootNodeTree',
				contentType: JSON,
				requestContentType: JSON)
	...
	def "B2B Admin gets a specific organizational unit: #orgUnitId"() {
		...
		def response = restClient.get(
				path: getBasePathWithSite() + '/users/' + B2B_ADMIN.id + ORGANIZATIONAL_UNITS_PATH + '/' + orgUnitId,
				query: [
	...
	def "B2B Admin tries to get a non-existing organizational unit"() {
		...
		with(response) {
			...
			data.errors[0].message == "Organizational unit with id [Non_Existing_Organizational_Unit] was not found"
		}
	...
	def "B2B Admin tries to get a specific organizational unit from another organizational unit"() {
		...
		with(response) {
			...
			status == SC_NOT_FOUND
			data.errors[0].type == "NotFoundError"
			data.errors[0].message == "Organizational unit with id [Pronto] was not found"
		}
	...
	def "B2B Customer tries to get a specific organizational unit"() {
		...
		when: "he requests to get a specific organizational unit"
		def response = restClient.get(
				path: getBasePathWithSite() + '/users/' + B2B_CUSTOMER.id + ORGANIZATIONAL_UNITS_PATH + "/" + orgUnitId,
				contentType: JSON,
				requestContentType: JSON)
	...
	def "B2B Admin edits a specific organizational unit: #orgUnitId"() {
		...
		def response = restClient.patch(
				path: getBasePathWithSite() + '/users/' + B2B_ADMIN.id + ORGANIZATIONAL_UNITS_PATH + "/" + orgUnitIdInRequest,
				body: patchBody,
				contentType: JSON,
				requestContentType: JSON)

		def updatedInfo = getOrganizationalUnit(restClient, B2B_ADMIN, orgUnitId)
	...
	def "B2B Admin tries to edit a specific organizational unit from different org: #orgUnitId"() {
		given: "a registered and logged in customer with B2B Admin role"
		authorizeCustomer(restClient, B2B_ADMIN)
		...
		where:
		orgUnitIdInRequest | orgUnitId              | name              | parent   | active | approvalProcessCode | patchBody
		"Pronto"           | "Pronto"               | "Pronto2"         | null     | true   | "accApproval"       | ["name": "Pronto2"]
	}

	def "B2B Customer tries to edit a specific organizational unit from different org: #orgUnitId"() {
		given: "a registered and logged in customer without B2B Admin role"
		authorizeCustomer(restClient, B2B_CUSTOMER)

		when: "he requests to edit a specific organizational unit"
		def response = restClient.patch(
			path: getBasePathWithSite() + '/users/' + B2B_CUSTOMER.id + ORGANIZATIONAL_UNITS_PATH + "/" + orgUnitIdInRequest,
			body: patchBody,
			contentType: JSON,
			requestContentType: JSON)
	...
	def "B2B Admin gets a list of available parent units for the given unit based on id"() {
		...
		def response = restClient.get(
				path: getBasePathWithSite() + '/users/' + B2B_ADMIN.id + ORGANIZATIONAL_UNITS_PATH + "/" + orgUnitId + "/availableParents",
				contentType: JSON,
				requestContentType: JSON)
	...
	def "B2B Admin tries to edit a non-existing organizational unit: #orgUnitId"() {
		...
		with(response) {
			...
			status == SC_NOT_FOUND
			data.errors[0].type == "NotFoundError"
			data.errors[0].message == "Organizational unit with id [Non_Existing_Unit] was not found"
		}
	...
	def "B2B Admin tries to edit a specific organizational unit with non-valid attributes: #descriptor"() {
		...
		"Rustic"  | "Long_Id"    | ["uid": createTestString(256)]
		"Rustic"  | "Long_Name"  | ["name": createTestString(256)]
	}
	...
	def "B2B Admin enables/disables a specific organizational unit from different org unit: #orgUnitId"() {
		given: "a registered and logged in customer with B2B Admin role"
		authorizeCustomer(restClient, B2B_ADMIN)
		...
		then: "activation/deactivation is not applied"
		with(response) {
			status == SC_NOT_FOUND
			data.errors[0].type == "NotFoundError"
			data.errors[0].message == "Organizational unit with id [Pronto] was not found"
		...
		where:
		orgUnitIdInRequest | orgUnitId   | name     | parent  | active | approvalProcessCode | patchBody
		"Pronto"           | "Pronto"    | "Pronto" | null    | true   | "accApproval"       | ["active": true]
	}
	...
	def "B2B Admin creates a new organizational unit: #orgUnitId"() {
		...
		def response = restClient.post(
				path: getBasePathWithSite() + '/users/' + B2B_ADMIN.id + ORGANIZATIONAL_UNITS_PATH,
				body: postBody,
				contentType: JSON,
				requestContentType: JSON)
	...
	def "B2B Admin tries to create a new organizational unit with non-valid attributes: #orgUnitId"() {
		...
		"Rustic_Long_Id"    | "Please select a unique identifier for the business unit" | "invalid" | "uid"   | "parameter" | "ValidationError" | ["approvalProcess": ["code": "accApproval"], "uid": createTestString(256), "name": "", "parentOrgUnit": ["uid": "Rustic_Retail"]]
		...
		"Rustic_Long_Name"  | "Please select a name for the business unit"              | "invalid" | "name"  | "parameter" | "ValidationError" | ["approvalProcess": ["code": "accApproval"], "uid": "Rustic_Long_Name", "name": createTestString(256), "parentOrgUnit": ["uid": "Rustic_Retail"]]
	}
	...
	def "B2B Customer tries to create a new order approval permission: #orgUnitId"() {
		...
		when: "he requests to create a new organizational unit"
		def response = restClient.post(
				path: getBasePathWithSite() + '/users/' + B2B_CUSTOMER.id + ORGANIZATIONAL_UNITS_PATH,
				body: postBody,
				contentType: JSON,
				requestContentType: JSON)
	...
	def "B2B Admin gets available approval processes"() {
		...
		when: "he requests to get the root organizational unit node of his organizational unit"
		def response = restClient.get(
				path: getBasePathWithSite() + '/users/' + B2B_ADMIN.id + '/orgUnitsAvailableApprovalProcesses',
				contentType: JSON,
				requestContentType: JSON)
	...
	def "B2B Customer tries to get available approval processes"() {
		...
		when: "he requests to get the root organizational unit node of his organizational unit"
		def response = restClient.get(
				path: getBasePathWithSite() + '/users/' + B2B_CUSTOMER.id + '/orgUnitsAvailableApprovalProcesses',
				contentType: JSON,
				requestContentType: JSON)
	...
	def "B2B Admin gets available organizational unit nodes"() {
		...
		def response = restClient.get(
				path: getBasePathWithSite() + '/users/' + B2B_ADMIN.id + '/availableOrgUnitNodes',
				contentType: JSON,
				requestContentType: JSON)
	...
	def "B2B Customer tries to get available organizational unit nodes"() {
		...
		when: "he requests to get available organizational unit nodes"
		def response = restClient.get(
				path: getBasePathWithSite() + '/users/' + B2B_CUSTOMER.id + '/availableOrgUnitNodes',
				contentType: JSON,
				requestContentType: JSON)
	...
	def "B2B Customer tries to get an organizational unit's addresses: #orgUnitId"() {
		...
		def response = restClient.get(
				path: getBasePathWithSite() + '/users/' + B2B_CUSTOMER.id + ORGANIZATIONAL_UNITS_PATH + '/' + orgUnitId + '/addresses',
				contentType: JSON,
				requestContentType: JSON)
	...
	def "B2B Admin gets an organizational unit's addresses: #orgUnitId"() {
		...
		def response = restClient.get(
				path: getBasePathWithSite() + '/users/' + B2B_ADMIN.id + ORGANIZATIONAL_UNITS_PATH + '/' + orgUnitId + '/addresses',
				contentType: JSON,
				requestContentType: JSON)
	...
	def "B2B Admin edits a specific address from an organizational unit: #line1"() {
		given: "a registered and logged in customer with B2B Admin role"
		authorizeCustomer(restClient, B2B_ADMIN)
		def currentUnit = getOrganizationalUnit(restClient, B2B_ADMIN, orgUnitId).data

		def addressId = currentUnit.addresses.find { it.line1 == originalLine1 }.id

		when: "he requests to edit a specific address"
		def response = restClient.patch(
				path: getBasePathWithSite() + '/users/' + B2B_ADMIN.id + ORGANIZATIONAL_UNITS_PATH + "/" + orgUnitId + '/addresses/' + addressId,
				body: patchBody,
				contentType: JSON,
				requestContentType: JSON)
	...
	def "B2B Customer tries to edit a specific address from an organizational unit: #orgUnitId"() {
		given: "a registered and logged in customer without B2B Admin role"
		authorizeCustomer(restClient, B2B_CUSTOMER)

		when: "he requests to edit a specific address"
		def response = restClient.patch(
				path: getBasePathWithSite() + '/users/' + B2B_CUSTOMER.id + ORGANIZATIONAL_UNITS_PATH + "/" + orgUnitId + '/addresses/' + addressId,
				body: patchBody,
				contentType: JSON,
				requestContentType: JSON)
	...
	def "B2B Admin gets users related to an organizational unit based on role: #orgUnitId #roleId"() {
		...
		def response = restClient.get(
				path: getBasePathWithSite() + '/users/' + B2B_ADMIN.id + ORGANIZATIONAL_UNITS_PATH + "/" + orgUnitId + '/availableUsers/' + roleId,
				contentType: JSON,
				requestContentType: JSON)
	...
	def "B2B Customer gets users related to an organizational unit based on role"() {
		...
		when: "he requests to get users related to an organizational unit based on role"
		def response = restClient.get(
				path: getBasePathWithSite() + '/users/' + B2B_CUSTOMER.id + ORGANIZATIONAL_UNITS_PATH + "/" + orgUnitId + '/availableUsers/' + roleId,
				contentType: JSON,
				requestContentType: JSON)
	...
	def "B2B Admin removes a role from a specific user: #orgUnitId #userId #roleId"() {
		given: "a registered and logged in customer with B2B Admin role"
		authorizeCustomer(restClient, B2B_ADMIN)
		def currentUnit = getOrganizationalUnit(restClient, B2B_ADMIN, orgUnitId)

		when: "he requests to remove a role from a user "
		def response = restClient.delete(
				path: getBasePathWithSite() + '/users/' + B2B_ADMIN.id + '/orgCustomers/' + customerId + "/roles/" + roleId,
				contentType: JSON,
				requestContentType: JSON)

		def updatedUnit = getOrganizationalUnit(restClient, B2B_ADMIN, orgUnitId)
	...
	def "B2B Customer tries to remove a role from a specific user"() {
		...
		when: "he requests to remove a role from a user "
		def response = restClient.delete(
				path: getBasePathWithSite() + '/users/' + B2B_CUSTOMER.id + '/orgCustomers/' + customerId + "/roles/" + roleId,
				contentType: JSON,
				requestContentType: JSON)
	...
	def "B2B Admin adds a role to a user: #orgUnitId #userId #roleId"() {
		...
		def response = restClient.post(
				path: getBasePathWithSite() + '/users/' + B2B_ADMIN.id + '/orgCustomers/' + customerId + "/roles",
				query: ["roleId": roleId],
				contentType: JSON,
				requestContentType: JSON)
	...
	def "B2B Customer tries to add a role to a user"() {
		...
		when: "he requests to add a role to a user"
		def response = restClient.post(
				path: getBasePathWithSite() + '/users/' + B2B_CUSTOMER.id + '/orgCustomers/' + customerId + "/roles",
				query: ["roleId": roleId],
				contentType: JSON,
				requestContentType: JSON)
	...
	def "B2B Admin removes a specific user from an organizational unit related role: #orgUnitId #userId #roleId"() {
		...
		when: "he requests to remove a role from a user "
		def response = restClient.delete(
				path: getBasePathWithSite() + '/users/' + B2B_ADMIN.id + ORGANIZATIONAL_UNITS_PATH + "/" + orgUnitId + '/orgCustomers/' + customerId + "/roles/" + roleId,
				contentType: JSON,
				requestContentType: JSON)
	...
	def "B2B Customer tries to remove a specific user from a  organizational unit dependent role"() {
		...
		when: "he requests to remove a role from a user "
		def response = restClient.delete(
				path: getBasePathWithSite() + '/users/' + B2B_CUSTOMER.id + ORGANIZATIONAL_UNITS_PATH + "/" + orgUnitId + '/orgCustomers/' + customerId + "/roles/" + roleId,
				contentType: JSON,
				requestContentType: JSON)
	...
	def "B2B Admin adds an organizational unit dependent role to a user: #orgUnitId #userId #roleId"() {
		...
		when: "he requests to add a role to a user"
		def response = restClient.post(
				path: getBasePathWithSite() + '/users/' + B2B_ADMIN.id + ORGANIZATIONAL_UNITS_PATH + '/' + orgUnitId + '/orgCustomers/' + customerId + "/roles",
				query: ["roleId": roleId],
				contentType: JSON,
				requestContentType: JSON)
	...
	def "B2B Customer tries to add an organizational unit dependent role role to a user"() {
		...
		when: "he requests to add a role to a user"
		def response = restClient.post(
				path: getBasePathWithSite() + '/users/' + B2B_CUSTOMER.id + ORGANIZATIONAL_UNITS_PATH + '/' + orgUnitId + '/orgCustomers/' + customerId + "/roles",
				query: ["roleId": roleId],
				contentType: JSON,
				requestContentType: JSON)
