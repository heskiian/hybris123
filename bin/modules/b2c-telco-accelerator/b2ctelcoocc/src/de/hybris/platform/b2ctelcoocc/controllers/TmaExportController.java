...
public class TmaExportController extends BaseController
{
	...
	static
	{
		...
		productOptions = productOptions.trim().replace(" ", B2ctelcooccConstants.OPTIONS_SEPARATOR);
		OPTIONS = extractOptions(productOptions);
	}

	protected static Set<ProductOption> extractOptions(final String options)
	{
		final List<String> optionsStrings = Arrays.asList(options.split(B2ctelcooccConstants.OPTIONS_SEPARATOR));
	...
	@RequestMappingOverride(priorityProperty = "b2ctelcoocc.TmaExportController.exportProducts.priority")
	@ResponseBody
