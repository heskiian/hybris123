...
class OrgUnitUserGroupsControllerTest extends AbstractUserTest {
	...
	def "B2B Admin gets organizational unit user groups"() {
		...
		def response = restClient.get(
				path: getBasePathWithSite() + '/users/' + B2B_ADMIN.id + ORGANIZATIONAL_USER_GROUPS_PATH,
				contentType: JSON,
	...
	def "B2B Customer tries to get organizational unit user groups"() {
		...
		def response = restClient.get(
				path: getBasePathWithSite() + '/users/' + B2B_CUSTOMER.id + ORGANIZATIONAL_USER_GROUPS_PATH,
				contentType: JSON,
				requestContentType: JSON)
	...
	def "B2B Admin gets a specific organizational unit user group: #orgUnitUserGroupId"() {
		...
		def response = restClient.get(
				path: getBasePathWithSite() + '/users/' + B2B_ADMIN.id + ORGANIZATIONAL_USER_GROUPS_PATH + '/' + orgUnitUserGroupId,
				query: [
	...
	def "B2B Admin tries to get a non-existing organizational unit user group"() {
		...
		with(response) {
			...
			data.errors[0].message == "Member Permission not found"
		}
	...
	def "B2B Customer tries to get a specific organizational unit user group"() {
		...
		when: "he requests to get a specific organizational unit user group"
		def response = restClient.get(
				path: getBasePathWithSite() + '/users/' + B2B_CUSTOMER.id + ORGANIZATIONAL_USER_GROUPS_PATH + "/" + orgUnitUserGroupId,
				contentType: JSON,
				requestContentType: JSON)
	...
	def "B2B Admin edits a specific organizational unit user group: #orgUnitUserGroupId"() {
		...
		def response = restClient.patch(
				path: getBasePathWithSite() + '/users/' + B2B_ADMIN.id + ORGANIZATIONAL_USER_GROUPS_PATH + "/" + orgUnitUserGroupIdInRequest,
				body: patchBody,
				contentType: JSON,
				requestContentType: JSON)

		def updatedInfo = getOrganizationalUnitUserGroup(restClient, B2B_ADMIN, orgUnitUserGroupId)
	...
	def "B2B Customer tries to edit a specific organizational unit user group: #orgUnitUserGroupIdInRequest"() {
		given: "a registered and logged in customer without B2B Admin role"
		authorizeCustomer(restClient, B2B_CUSTOMER)

		when: "he requests to edit a specific organizational unit user group:"
		def response = restClient.patch(
				path: getBasePathWithSite() + '/users/' + B2B_CUSTOMER.id + ORGANIZATIONAL_USER_GROUPS_PATH + "/" + orgUnitUserGroupIdInRequest,
				body: patchBody,
				contentType: JSON,
				requestContentType: JSON)
	...
	def "B2B Admin tries to edit a specific organizational unit user group with non-valid attributes: #descriptor"() {
		...
		"standardPermissions"       | "Long_Id"     | SC_BAD_REQUEST | "Please select a unique identifier for the user group" | "ValidationError"    | ["uid": createTestString(256)]
		"standardPermissions"       | "Long_Name"   | SC_BAD_REQUEST | "Please select a name for the user group"              | "ValidationError"    | ["name":  createTestString(256)]
	...
	def "B2B Admin removes all members of a specific organizational unit user group: #orgUnitUserGroupId"() {
		given: "a registered and logged in customer with B2B Admin role"
		authorizeCustomer(restClient, B2B_ADMIN)
		def currentOrgUnitUserGroup = getOrganizationalUnitUserGroup(restClient, B2B_ADMIN, orgUnitUserGroupId)

		when: "he requests to remove all members of a specific organizational unit user group"
		def response = restClient.delete(
				path: getBasePathWithSite() + '/users/' + B2B_ADMIN.id + ORGANIZATIONAL_USER_GROUPS_PATH + "/" + orgUnitUserGroupId + "/members",
				contentType: JSON,
				requestContentType: JSON)

		def updatedOrgUnitUserGroup = getOrganizationalUnitUserGroup(restClient, B2B_ADMIN, orgUnitUserGroupId)
	...
	def "B2B Customer tries to remove all members of a specific organizational unit user group: #orgUnitUserGroupId"() {
		...
		when: "he requests to remove all members of a specific organizational unit user group"
		def response = restClient.delete(
				path: getBasePathWithSite() + '/users/' + B2B_CUSTOMER.id + ORGANIZATIONAL_USER_GROUPS_PATH + "/" + orgUnitUserGroupId + "/members",
				contentType: JSON,
				requestContentType: JSON)
	...
	def "B2B Admin tries to create a new organizational unit user group with non-valid attributes: #descriptor"() {
		...
		"UserGroup_Long_Id"        | SC_BAD_REQUEST | "Please select a unique identifier for the user group" | "ValidationError"    | ["uid": createTestString(256), "name": "Standard Permissions","orgUnit": ["uid": "Rustic"]]
		...
		"UserGroup_Long_Name"      | SC_BAD_REQUEST | "Please select a name for the user group"              | "ValidationError"    | ["uid": "standardPermissions", "name": createTestString(256),"orgUnit": ["uid": "Rustic"]]
	...
	def "B2B Admin gets users related to the organizational unit user group: #orgUnitUserGroupId"() {
		...
		def response = restClient.get(
				path: getBasePathWithSite() + '/users/' + B2B_ADMIN.id + ORGANIZATIONAL_USER_GROUPS_PATH + "/" + orgUnitUserGroupId + '/availableOrgCustomers',
				contentType: JSON,
				requestContentType: JSON)
	...
	def "B2B Customer gets users related to the organizational unit user group"() {
		...
		def response = restClient.get(
				path: getBasePathWithSite() + '/users/' + B2B_CUSTOMER.id + ORGANIZATIONAL_USER_GROUPS_PATH + "/" + orgUnitUserGroupId + '/availableOrgCustomers',
				contentType: JSON,
				requestContentType: JSON)
	...
	def "B2B Admin tries to get users related to an organizational unit user group with invalid parameters: #descriptor"() {
		...
		with(response) {
			...
			data.errors[0].message == "No usergroup found for uid Non_Existing_Org_Unit_User_Group"
		}
		where:
	...
	def "B2B Admin removes a specific organizational unit user group from a user: #orgUnitUserGroupId #userId"() {
		...
		def response = restClient.delete(
				path: getBasePathWithSite() + '/users/' + B2B_ADMIN.id + ORGANIZATIONAL_USER_GROUPS_PATH + '/' + orgUnitUserGroupId + '/members/' + customerID,
				contentType: JSON,
				requestContentType: JSON)
	...
	def "B2B Customer tries to remove an organizational customer from organizational user group members"() {
		...
		when: "he requests to remove an organizational customer from organizational user group members"
		def response = restClient.delete(
				path: getBasePathWithSite() + '/users/' + B2B_CUSTOMER.id + ORGANIZATIONAL_USER_GROUPS_PATH + '/' + orgUnitUserGroupId + '/members/' + customerID,
				contentType: JSON,
				requestContentType: JSON)
	...
	def "B2B Admin adds a permission to an organizational unit user group: #orgUnitUserGroupId #permissionCode"() {
		...
		def response = restClient.post(
				path: getBasePathWithSite() + '/users/' + B2B_ADMIN.id + ORGANIZATIONAL_USER_GROUPS_PATH + "/" + orgUnitUserGroupId + "/orderApprovalPermissions",
	...
	def "B2B Customer tries to add a permission to an organizational unit user group"() {
		...
		def response = restClient.post(
				path: getBasePathWithSite() + '/users/' + B2B_CUSTOMER.id + ORGANIZATIONAL_USER_GROUPS_PATH + "/" + orgUnitUserGroupId + "/orderApprovalPermissions",
				query: ["orderApprovalPermissionCode": permissionCode],
				contentType: JSON,
				requestContentType: JSON)
	...
	def "B2B Admin gets permissions related to the organizational unit user group: #orgUnitUserGroupId"() {
		...
		def response = restClient.get(
				path: getBasePathWithSite() + '/users/' + B2B_ADMIN.id + ORGANIZATIONAL_USER_GROUPS_PATH + "/" + orgUnitUserGroupId + '/availableOrderApprovalPermissions',
				contentType: JSON,
				requestContentType: JSON)
	...
	def "B2B Customer gets permissions related to the organizational unit user group"() {
		...
		def response = restClient.get(
				path: getBasePathWithSite() + '/users/' + B2B_CUSTOMER.id + ORGANIZATIONAL_USER_GROUPS_PATH + "/" + orgUnitUserGroupId + '/availableOrderApprovalPermissions',
				contentType: JSON,
				requestContentType: JSON)
	...
	def "B2B Admin tries to get permissions related to an organizational unit user group with invalid parameters: #descriptor"() {
		...
		with(response) {
			status == SC_BAD_REQUEST
			data.errors[0].type == "IllegalArgumentError"
			data.errors[0].message == "No user group found for uid Non_Existing_Org_Unit_User_Group"
		}
		where:
		descriptor                         | orgUnitUserGroupId
		"Non_Existing_Org_Unit_User_Group" | "Non_Existing_Org_Unit_User_Group"
	...
	def "B2B Admin removes a specific permission from a organizational user group: #orgUnitUserGroupId #permissionCode"() {
		...
		def response = restClient.delete(
				path: getBasePathWithSite() + '/users/' + B2B_ADMIN.id + ORGANIZATIONAL_USER_GROUPS_PATH + "/" + orgUnitUserGroupId + "/orderApprovalPermissions/" + permissionCode,
				contentType: JSON,
				requestContentType: JSON)
	...
	def "B2B Customer tries to remove a specific permission from an organizational unit user group"() {
		...
		def response = restClient.delete(
				path: getBasePathWithSite() + '/users/' + B2B_CUSTOMER.id + ORGANIZATIONAL_USER_GROUPS_PATH + "/" + orgUnitUserGroupId + "/orderApprovalPermissions/" + permissionCode,
				contentType: JSON,
				requestContentType: JSON)
