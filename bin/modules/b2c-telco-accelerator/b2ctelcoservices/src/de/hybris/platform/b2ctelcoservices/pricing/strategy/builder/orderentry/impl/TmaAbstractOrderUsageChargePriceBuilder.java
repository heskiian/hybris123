 ...
 * @since 1907
 */
public class TmaAbstractOrderUsageChargePriceBuilder extends DefaultTmaAbstractOrderEntryPriceBuilder
{
	@Override
		...
		if (CollectionUtils.isNotEmpty(price.getUsageCharges()))
		{
			price.getUsageCharges().forEach(usageCharge ->
			{
				...
				if (usageCharge instanceof PerUnitUsageChargeModel)
				{
					...
					((TmaAbstractOrderTierUsageCompositePriceModel) usageChargePrice)
							.setUsageChargeType(((PerUnitUsageChargeModel) usageCharge).getUsageChargeType());
				}
				else
				{
